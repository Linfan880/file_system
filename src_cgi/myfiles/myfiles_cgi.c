...
/*json数据如下
    ...
    */
int get_count_json_info(const char *buf, char *user, char *token){
    int ret = 0;
    //解析json包
    //解析一个json字符串为cJSON对象
...
void return_login_status(long num, int token_flag)
{
    ...
    cJSON *root = cJSON_CreateObject();             //创建json句柄
...
//    TODO:优化，不能写死
    ...
    LOG(MYFILES_LOG_MODULE, MYFILES_LOG_PROC, "msql conn success!!!\n");
...
int main(){
    ...
    while (FCGI_Accept() >= 0)
    {
        ...
        if (len <= 0)
        {
            printf("Myfiles Module: No data from standard input.<p>\n");
            LOG(MYFILES_LOG_MODULE, MYFILES_LOG_PROC, "Myfiles Module: len = 0, No data from standard input\n");
        }
        else
        {
            char buf[4*1024] = {0};
