...
int get_md5_info(const char *buf, char *user, char *token, char *md5, char *filename){
    int ret=0;
    ...
    strcpy(token,   cJSON_GetObjectItem(root,"token")->valuestring);
    strcpy(md5,     cJSON_GetObjectItem(root,"md5")->valuestring);
    strcpy(filename,cJSON_GetObjectItem(root,"filename")->valuestring);
...
int process_result_one(MYSQL *conn, const char *sql_cmd, char *buf)
{
    ...
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "mysql_query err\n");
        ret = -1;
        goto END;
    }
    ...
    if (res_set == NULL)
    {
        LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "mysql_store_result err\n");
        ret = -1;
        goto END;
    }
...
//    TODO:优化，不能写死
    ...
    LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "msql conn success!!!\n");
...
int main(){
    while(FCGI_Accept() >= 0){
        ...
        if (len <= 0)//没有数据
        {
            printf("MD5 Module: No data from standard input.<p>\n");
            LOG(MD5_LOG_MODULE, MD5_LOG_PROC, "MD5 Module: len = 0, No data from standard input\n");
        }
        else //获取登陆用户信息
        {
            char buf[4 * 1024] = {0};
